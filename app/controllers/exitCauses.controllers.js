...
exports.updateOrCreateExitCauses = (req, res) => {
  if (!req.body) {
    res.status(400).json({ err: "not found data" });
  } else {
   req.body.forEach(function (el,index){
      var obj = new ExitCauses({
         exitCausesTitle: el.exitCausesTitle,
         studentId: el.studentId,
         createdBy: el.createdBy,
       });
        ExitCauses.updateOrCreate(obj, (err, reslut) => {
         if (err) {
           res.status(err.code).json(err);
         }  /* end of if */
       }); /* /updateOrCreateExitCauses models */
   })/* end of forEach */
  } /* end of if */
}; /* /updateOrCreateExitCauses function */
